...
pub unsafe extern "C" fn tun2proxy_set_traffic_status_callback(
    ...
    if let Ok(mut cb) = TRAFFIC_STATUS_CALLBACK.lock() {
        *cb = callback.map(|callback| TrafficStatusCallback(Some(callback), ctx));
    } else {
...
pub fn reset_traffic_status() {
    if let Ok(mut status) = TRAFFIC_STATUS.lock() {
        *status = TrafficStatus::default();
    }
    if let Ok(mut timestamp) = TIME_STAMP.lock() {
