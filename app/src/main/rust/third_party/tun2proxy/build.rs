fn main() -> Result<(), Box<dyn std::error::Error>> {
    // TunnelLens vendors the reviewed v0.8.2 source archive. Keep version
    // metadata deterministic instead of reading an unrelated parent Git tree
    // or embedding the local build time.
    println!("cargo:rustc-env=GIT_HASH=eed123f");
    println!("cargo:rustc-env=BUILD_TIME=source-v0.8.2");
