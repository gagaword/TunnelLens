...
#ifdef MODERNSOCKS_IPV4_TCP_ONLY
        /* ModernSocks Phase 3 rejects unsupported traffic before lwIP.
         * Rejecting UDP only at session construction is too late and can
         * keep the cooperative scheduler busy inside udp_input. */
        if (buf->len < 20 ||
            (((const unsigned char *)buf->payload)[0] >> 4) != 4 ||
            ((const unsigned char *)buf->payload)[9] != 6) {
            pbuf_free (buf);
